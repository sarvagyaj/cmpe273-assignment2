...
public class BookRepository implements BookRepositoryInterface {
	...
	// adds the book if the book is not present else updates the status of the
	// book from lost to available
	@Override
	public void updateBook(Book book) {
		if (!bookInMemoryMap.containsKey(book.getIsbn())) {
			bookInMemoryMap.put(book.getIsbn(), book);
			bookInMemoryMap.get(book.getIsbn()).setStatus(Status.available);
			System.out.println("Added book is : "
					+ bookInMemoryMap.get(book.getIsbn()));
		} else {
			String status = bookInMemoryMap.get(book.getIsbn()).getStatus()
					.getValue();
			if (bookInMemoryMap.containsKey(book.getIsbn())
					&& status.equalsIgnoreCase("lost")) {
				bookInMemoryMap.get(book.getIsbn()).setStatus(Status.available);
				System.out.println("Updated book is : "
						+ bookInMemoryMap.get(book.getIsbn()).getStatus()
								.getValue());
			} else {
